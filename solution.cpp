...
- next target = 2, scan the entire set {1,2,3} to calculate dp[2].
  - dp[2] = dp[2-1] + dp[2-2] + dp[2-3]
          = dp[1] + dp[0] (dp[2-3] is ignored)
  - There are two possibilities:
    - start from 0 and add a 2 {2}
    - start from 1 and add another 1 {1,1}
    - so dp[2] = dp[0(2-2)] + dp[1(2-1)] = 1 + 1 = 2
...
int Solution::find(vector<int> &nums, int target)
{
  ...
  for (auto t = 1; t <= target; t++)
    for (auto num : nums)
      if (t - num >= 0)
        /* every sequence that has its members summing up to t-num
           can be expended by adding "num" to get a sum of t
        */
        dp[t] += dp[t - num];
